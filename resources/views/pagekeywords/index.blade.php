<x-app-layout>
    <div class="mx-auto w-[98%] lg:ml-12">
        <div class="flex justify-between items-center w-[90%] mx-auto my-8">
            <h1 class="text-2xl font-bold">Page Keywords List</h1>
        ...
        <div class="bg-white shadow-lg rounded-lg p-8 w-full overflow-x-auto">
            ...
            <table id="example" class="display w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <form action="{{ route('pagekeywords.update', $pagekeyword->id) }}" method="POST" class="update-form">
                            @csrf
                            @method('PUT')
                            <td>
                                <input type="text" name="sectionname" class="w-full border border-gray-300 rounded px-2 py-1" value="{{ $pagekeyword->sectionname }}" disabled />
                            </td>
                            <td>
                                <textarea name="keywords" rows="2" class="w-full border border-gray-300 rounded px-2 py-1" disabled>{{ $pagekeyword->keywords }}</textarea>
                            </td>
                        ...
                        <form action="{{ route('pagekeywords.destroy', $pagekeyword->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
        ...
        <div class="relative w-full max-w-2xl mx-auto p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex justify-between items-center p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Add Page Keyword</h3>
                    <button type="button"
                ...
                <!-- Modal Body -->
                <form action="{{ route('pagekeywords.store') }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <textarea name="keywords" rows="3" class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="Keywords" required></textarea>
                    </div>
