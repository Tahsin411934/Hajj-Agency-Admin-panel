<x-app-layout>
    <div class="mx-auto w-[98%] lg:ml-12">
        <div class="flex justify-between items-center w-[90%] mx-auto my-8">
            <h1 class="text-2xl font-bold">Blog Content List</h1>
        ...
        <div class="bg-white shadow-lg rounded-lg p-8 w-full overflow-x-auto">
            ...
            <table id="example" class="display w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <form action="{{ route('blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="update-form">
                            @csrf
                            @method('PUT')
                            <td>
                                @if ($blog->image)
                                    <img src="{{ asset('storage/'.$blog->image) }}" alt="Blog Image" class="w-16 h-16 object-cover rounded-lg mt-2" />
                                @else
                                    <span>No Image</span>
                                @endif
                            </td>
                            <td>
                                <input type="text" name="title" class="w-full border border-gray-300 rounded px-2 py-1" value="{{ $blog->title }}" disabled />
                            </td>
                            <td>
                                <input type="text" name="shortdescription" class="w-full border border-gray-300 rounded px-2 py-1" value="{{ $blog->shortdescription }}" disabled />
                            </td>
                            <td>
                                <textarea name="details" rows="2" class="w-full border border-gray-300 rounded px-2 py-1" disabled>{{ $blog->details }}</textarea>
                            </td>
        ...
        <div class="relative w-full max-w-2xl mx-auto p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex justify-between items-center p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Add Blog Content</h3>
                    <button type="button"
                ...
                <!-- Modal Body -->
                <form action="{{ route('blogs.store') }}" method="POST" class="p-6 space-y-6" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <input type="text" name="shortdescription" class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="Short Description" required />
                        <textarea name="details" rows="3" class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="Details" required></textarea>
                    </div>
